function CareerPage() {
  return (
    <div className="flex-1 overflow-y-scroll p-7 md:overflow-y-auto lg:p-14 xl:p-24 xl:px-56">
      <h1 className="mb-3 text-3xl font-bold text-sky-900 lg:mb-7 lg:text-5xl">
        Career
      </h1>
      <p className="text-md mb-10 text-sky-900 lg:text-xl">
        Here is a brief look at some of the experiences and achievement I've had
        on my tech journey.
      </p>
      <img src="WSU_logo.svg.png" className="mb-5 h-10 lg:h-12"></img>
      <h2 className="text-xl font-bold text-sky-900 lg:text-2xl">
        Bachelor of Computer Science
      </h2>
      <p className="text-md mb-2 font-semibold text-stone-500 lg:mb-5">
        Jan 2017 - Aug 2021
      </p>
      <p className="text-md mb-10 text-sky-900 lg:mb-14 lg:text-xl">
        Comprehensive courses across multiple disciplines of Computer Science
        including Mobile Application Development, Artificial Intelligence,
        Systems Programming, Robotics, Web Development, Wireless Networking and
        more.
        <br />
        <br /> Through guided partical learning and direct communication with
        industry leaders, I developed strong foundational knowledge on
        fundamental concepts such as Data Structures, Algorithms, Discrete
        Mathematics, Internet Protocols, Objective Oriented Programming and
        other programming paradigms.
      </p>

      <img src="IBM_logo.svg.png" className="mb-5 h-8 lg:h-10"></img>
      <h2 className="text-xl font-bold text-sky-900 lg:text-2xl">
        IBM Chicago
      </h2>
      <p className="text-md mb-2 font-semibold text-stone-500 lg:mb-5">
        Jun 2020 - Dec 2020
      </p>
      <p className="text-md mb-10 text-sky-900 lg:mb-14 lg:text-xl">
        A volunteer project as part of my academics at Western Sydney University
        which involved contributing to the IBM Watson Astronomy Assistant app.
        The app was built in React Native and would provide a IM like interface
        where users could interface with the IBM Watson API to gather real-time
        astronomy related information with photo integration and other forms of
        media.
        <br />
        <br /> My contributions involved accessibility features such as TTS,
        Dictation and various themes for vision impaired users. The app was
        presented to a team of IBM Chicago engineers who were overall impressed
        with the teams efforts.
      </p>
      <img src="Infosys_logo.svg.png" className="mb-3 h-9 lg:h-10"></img>
      <h2 className="text-xl font-bold text-sky-900 lg:text-2xl">
        Infosys Limited Australia
      </h2>
      <p className="text-md mb-2 font-semibold text-stone-500 lg:mb-5">
        Dec 2023 - Current
      </p>
      <p className="text-md mb-10 text-sky-900 lg:mb-14 lg:text-xl">
        Lead Tester and QA Engineer on the Infosys SIT team, responsible for
        both functional and non-functional testing and test automation for
        Infosys clients. 2 years of experience using tools such as Selenium,
        Robot Framework, JMeter and LoadRunner to gather metrics accurately
        measuring the quality of software products.
        <br />
        <br /> Mentor and trainer for new testers as well as SME for existing
        testers across teams where I provide presentations and create
        documentation for project specific skills as well as global testing
        skills for testers to refer to.
        <br />
        <br /> Automated test case creation and test evidence creation using VBA
        macros and powershell scripts which improved the tester experience,
        reduced test preparation efforts and test execution times
      </p>
    </div>
  );
}
export default CareerPage;
